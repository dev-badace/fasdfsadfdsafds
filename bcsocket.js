import { getManager, on, decodeBind, getQuery } from "./setup.js";
...
async function connectBcGameCrash() {
  const query = await getQuery();

  const manager = getManager("wss://socket2v2.bcgame.top/", query);

  const game = manager.socket("/g/cm");

  // game.on("reconnection_attempt", async () => {
  //   const query = await getQuery();
  //   game.io.opts.query = query;
  // });
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`[bcGame] SSE Server running on port ${PORT}`);
});
